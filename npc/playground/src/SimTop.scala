...
class SimTop extends Module {
  ...
  val clkcount = RegInit(0.U(log2Ceil(counterMax50hz).W))
  val clk1scount = RegInit(0.U(log2Ceil(counterMax1s).W))
  val clk10scount = RegInit(0.U(log2Ceil(10).W))
  val clk100scount = RegInit(0.U(log2Ceil(10).W))
  ...
  when(clkcount === counterMax50hz.U) {
    ...
    when(clk1scount === counterMax1s.U) {
      ...
      when(clk10scount === 10.U) {
        clk10scount := 0.U
        clk100scount := clk100scount + 1.U

        when(clk100scount === 10.U) {
          clk100scount := 0.U
        }
      }
    }
  ...
  seg1.seg.in := Cat(clk100scount, clk10scount(3, 0))
  seg2.seg.in := Cat(clk1scount(3, 0), clk1scount(3, 0))

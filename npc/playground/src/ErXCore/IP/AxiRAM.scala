...
class Axi4FullSram extends ErXCoreModule {
...
class dpi_sram extends ErXCoreBlackBox with HasBlackBoxInline {
...
class reg_sram extends ErXCoreModule {
  val io = IO(new Bundle {
    val clock=Input(Clock())
  ...
  when(io.req){
    when(io.wr){
      ...
      io.rdata:=mem.read(0.U).asTypeOf(UInt(XLEN.W))
    }
  }
}

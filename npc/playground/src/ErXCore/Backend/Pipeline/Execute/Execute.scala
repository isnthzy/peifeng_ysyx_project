...
import ErXCore.DecodeInstructions.MRET
...
class Execute extends ErXCoreModule{
  ...
  for(i <- 0 until IssueWidth){
    ...
    io.fw_pr.upd(i).rfWen  := pipe(i).io.out.bits.rfWen & pipe(i).io.out.valid
...
class PipeMem(useDmem: Boolean = true) extends AbstaceExecutePipe(useDmem = useDmem){

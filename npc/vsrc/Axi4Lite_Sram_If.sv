...
module Axi4Lite_Sram_If(	// @[<stdin>:1998:3]
  input         clock,	// @[<stdin>:1999:11]
                reset,	// @[<stdin>:2000:11]
  ...
  wire       _io_aw_ready_output = 1'h1;	// @[<stdin>:1998:3, playground/src/LSFR.scala:28:12]
  wire       _io_w_ready_output = 1'h1;	// @[<stdin>:1998:3, playground/src/LSFR.scala:28:12]
  reg  [7:0] io_ar_ready_delay;	// @[playground/src/LSFR.scala:21:22]
  reg        io_ar_ready_data;	// @[playground/src/LSFR.scala:22:22]
  wire       _io_ar_ready_output = io_ar_ready_data;	// @[<stdin>:1998:3, playground/src/LSFR.scala:22:22]
  wire       _dpi_sram_io_req_T = _io_ar_ready_output & io_ar_valid;	// @[<stdin>:1998:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire       _dpi_sram_io_wr_T = _io_aw_ready_output & io_aw_valid;	// @[<stdin>:1998:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire       _dpi_sram_io_wr_T_1 = _io_w_ready_output & io_w_valid;	// @[<stdin>:1998:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  ...
  always @(posedge clock) begin	// @[<stdin>:1999:11]
    if (reset) begin	// @[<stdin>:1999:11]
      io_ar_ready_delay <= 8'h0;	// @[playground/src/LSFR.scala:21:22]
      io_ar_ready_data <= 1'h0;	// @[playground/src/LSFR.scala:22:{22,35}]
      readDataValidReg <= 1'h0;	// @[playground/src/LSFR.scala:22:35, playground/src/SramIF.scala:19:31]
      writeRespValidReg <= 1'h0;	// @[playground/src/LSFR.scala:22:35, playground/src/SramIF.scala:35:32]
    end
    else begin	// @[<stdin>:1999:11]
  ...
  assign io_ar_ready = _io_ar_ready_output;	// @[<stdin>:1998:3]
  assign io_r_valid = readDataValidReg;	// @[<stdin>:1998:3, playground/src/SramIF.scala:19:31]
  assign io_r_bits_resp = 2'h0;	// @[<stdin>:1998:3, playground/src/SramIF.scala:27:17]
  assign io_aw_ready = _io_aw_ready_output;	// @[<stdin>:1998:3]
  assign io_w_ready = _io_w_ready_output;	// @[<stdin>:1998:3]
  assign io_b_valid = writeRespValidReg;	// @[<stdin>:1998:3, playground/src/SramIF.scala:35:32]
  assign io_b_bits_resp = 2'h0;	// @[<stdin>:1998:3, playground/src/SramIF.scala:27:17]
endmodule

...
module Axi4Lite_Sram_Mem(	// @[<stdin>:1610:3]
  input         clock,	// @[<stdin>:1611:11]
                reset,	// @[<stdin>:1612:11]
  ...
  wire _io_ar_ready_output = 1'h1;	// @[<stdin>:1610:3, playground/src/Sram.scala:11:14]
  wire _io_aw_ready_output = 1'h1;	// @[<stdin>:1610:3, playground/src/Sram.scala:11:14]
  wire _io_w_ready_output = 1'h1;	// @[<stdin>:1610:3, playground/src/Sram.scala:11:14]
  wire _dpi_sram_io_req_T = _io_ar_ready_output & io_ar_valid;	// @[<stdin>:1610:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire _dpi_sram_io_wr_T = _io_aw_ready_output & io_aw_valid;	// @[<stdin>:1610:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire _dpi_sram_io_wr_T_1 = _io_w_ready_output & io_w_valid;	// @[<stdin>:1610:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  reg  readDataValidReg;	// @[playground/src/Sram.scala:17:31]
  reg  writeRespValidReg;	// @[playground/src/Sram.scala:35:32]
  always @(posedge clock) begin	// @[<stdin>:1611:11]
    if (reset) begin	// @[<stdin>:1611:11]
      readDataValidReg <= 1'h0;	// @[playground/src/Sram.scala:17:31]
      writeRespValidReg <= 1'h0;	// @[playground/src/Sram.scala:17:31, :35:32]
    end
    else begin	// @[<stdin>:1611:11]
  ...
  assign io_ar_ready = _io_ar_ready_output;	// @[<stdin>:1610:3]
  assign io_r_valid = readDataValidReg;	// @[<stdin>:1610:3, playground/src/Sram.scala:17:31]
  assign io_r_bits_resp = 2'h0;	// @[<stdin>:1610:3, playground/src/Sram.scala:25:17]
  assign io_aw_ready = _io_aw_ready_output;	// @[<stdin>:1610:3]
  assign io_w_ready = _io_w_ready_output;	// @[<stdin>:1610:3]
  assign io_b_valid = writeRespValidReg;	// @[<stdin>:1610:3, playground/src/Sram.scala:35:32]
  assign io_b_bits_resp = 2'h0;	// @[<stdin>:1610:3, playground/src/Sram.scala:25:17]
endmodule

...
module Axi4Lite_Sram_Mem(	// @[<stdin>:1818:3]
  input         clock,	// @[<stdin>:1819:11]
                reset,	// @[<stdin>:1820:11]
  ...
  wire       _dpi_sram_io_wr_T = _io_aw_ready_output & io_aw_valid;	// @[<stdin>:1818:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire       _io_w_ready_output;	// @[<stdin>:1818:3]
  wire       _dpi_sram_io_wr_T_1 = _io_w_ready_output & io_w_valid;	// @[<stdin>:1818:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  reg        readDataValidReg;	// @[playground/src/Sram.scala:18:31]
  reg  [3:0] io_aw_ready_delay;	// @[playground/src/LSFR.scala:20:22]
  reg        io_aw_ready_data;	// @[playground/src/LSFR.scala:21:22]
  assign _io_aw_ready_output = io_aw_ready_data;	// @[<stdin>:1818:3, playground/src/LSFR.scala:21:22]
  reg  [3:0] io_w_ready_delay;	// @[playground/src/LSFR.scala:20:22]
  ...
  always @(posedge clock) begin	// @[<stdin>:1819:11]
    if (reset) begin	// @[<stdin>:1819:11]
      ...
      io_aw_ready_delay <= 4'h0;	// @[playground/src/LSFR.scala:20:22]
      io_aw_ready_data <= 1'h0;	// @[playground/src/LSFR.scala:21:22, playground/src/Sram.scala:14:26]
      io_w_ready_delay <= 4'h0;	// @[playground/src/LSFR.scala:20:22]
      io_w_ready_data <= 1'h0;	// @[playground/src/LSFR.scala:21:22, playground/src/Sram.scala:14:26]
      ...
      if (_io_aw_ready_T)	// @[playground/src/LSFR.scala:24:15]
        ...
        io_aw_ready_delay <= io_aw_ready_delay - 4'h1;	// @[playground/src/LSFR.scala:20:22, :28:21]
      io_aw_ready_data <= _io_aw_ready_T | io_aw_ready_data;	// @[playground/src/LSFR.scala:21:22, :24:{15,23}, :25:12]
      if (_io_w_ready_T)	// @[playground/src/LSFR.scala:24:15]
        ...
        io_w_ready_delay <= io_w_ready_delay - 4'h1;	// @[playground/src/LSFR.scala:20:22, :28:21]
      io_w_ready_data <= _io_w_ready_T | io_w_ready_data;	// @[playground/src/LSFR.scala:21:22, :24:{15,23}, :25:12]
  ...
  assign io_r_bits_resp = 2'h0;	// @[<stdin>:1818:3, playground/src/Sram.scala:26:17]
  assign io_aw_ready = _io_aw_ready_output;	// @[<stdin>:1818:3]
  assign io_w_ready = _io_w_ready_output;	// @[<stdin>:1818:3]
  assign io_b_valid = writeRespValidReg;	// @[<stdin>:1818:3, playground/src/Sram.scala:37:32]
  assign io_b_bits_resp = 2'h0;	// @[<stdin>:1818:3, playground/src/Sram.scala:26:17]
endmodule

...
module Axi4Lite_Sram_Mem(	// @[<stdin>:1869:3]
  input         clock,	// @[<stdin>:1870:11]
                reset,	// @[<stdin>:1871:11]
  ...
  wire       _io_ar_ready_output = io_ar_ready_data;	// @[<stdin>:1869:3, playground/src/LSFR.scala:22:22]
  wire       _dpi_sram_io_req_T = _io_ar_ready_output & io_ar_valid;	// @[<stdin>:1869:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire       _io_aw_ready_output;	// @[<stdin>:1869:3]
  wire       _dpi_sram_io_wr_T = _io_aw_ready_output & io_aw_valid;	// @[<stdin>:1869:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire       _io_w_ready_output;	// @[<stdin>:1869:3]
  wire       _dpi_sram_io_wr_T_1 = _io_w_ready_output & io_w_valid;	// @[<stdin>:1869:3, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  ...
  assign _io_aw_ready_output = io_aw_ready_data;	// @[<stdin>:1869:3, playground/src/LSFR.scala:22:22]
  reg  [7:0] io_w_ready_delay;	// @[playground/src/LSFR.scala:21:22]
  reg        io_w_ready_data;	// @[playground/src/LSFR.scala:22:22]
  assign _io_w_ready_output = io_w_ready_data;	// @[<stdin>:1869:3, playground/src/LSFR.scala:22:22]
  ...
  always @(posedge clock) begin	// @[<stdin>:1870:11]
    if (reset) begin	// @[<stdin>:1870:11]
  ...
  assign io_ar_ready = _io_ar_ready_output;	// @[<stdin>:1869:3]
  assign io_r_valid = readDataValidReg;	// @[<stdin>:1869:3, playground/src/Sram.scala:18:31]
  assign io_r_bits_resp = 2'h0;	// @[<stdin>:1869:3, playground/src/Sram.scala:26:17]
  assign io_aw_ready = _io_aw_ready_output;	// @[<stdin>:1869:3]
  assign io_w_ready = _io_w_ready_output;	// @[<stdin>:1869:3]
  assign io_b_valid = writeRespValidReg;	// @[<stdin>:1869:3, playground/src/Sram.scala:37:32]
  assign io_b_bits_resp = 2'h0;	// @[<stdin>:1869:3, playground/src/Sram.scala:26:17]
endmodule

...
module CsrFile(	// @[<stdin>:1128:3]
  ...
  wire [31:0] _io_out_output =
    io_csr_addr == 12'h343
      ...
      : io_csr_addr == 12'h342
          ...
          : io_csr_addr == 12'h341
              ? mepc
              : io_csr_addr == 12'h300 ? mstatus : io_csr_addr == 12'h305 ? mtvec : 32'h0;	// @[playground/src/Csr.scala:36:18, :37:22, :38:18, :39:18, :40:18, :43:22, :45:37]
  ...
  always @(posedge clock) begin	// @[<stdin>:1129:11]
    ...
    if (~_GEN_2 | _GEN_6 | ~_GEN_5) begin	// @[playground/src/Csr.scala:37:22, :40:18, :59:27, :68:18, :69:36, :70:42, :71:42, :72:42, :73:{27,42}]
    end
    else if (_wdata_T_3)	// @[playground/src/Csr.scala:53:38]
    ...
    if (reset)	// @[<stdin>:1129:11]
      mstatus <= 32'h1800;	// @[playground/src/Csr.scala:37:22]
    else if (~_GEN_2 | _GEN_0 | ~_GEN_1) begin	// @[playground/src/Csr.scala:37:22, :59:27, :68:18, :69:{23,36}, :70:{27,42}]
    end
    else if (_wdata_T_3)	// @[playground/src/Csr.scala:53:38]
      ...
      mstatus <= 32'h0;	// @[playground/src/Csr.scala:37:22, :43:22]
  end // always @(posedge)
  csr_debug_dpic csr_dpic (	// @[playground/src/Csr.scala:78:22]
    .clock      (clock),
    .reset      (reset),
    .csr_valid  (|io_csr_cmd),	// @[playground/src/Csr.scala:59:18]
    .assert_wen (_GEN_2 & ~_GEN_6 & ~_GEN_5)	// @[playground/src/Csr.scala:36:18, :37:22, :40:18, :42:33, :59:27, :68:18, :69:36, :70:42, :71:42, :72:42, :73:{27,42}, :74:32]
  );

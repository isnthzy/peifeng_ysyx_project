...
module Axi4Lite_Sram_Bridge(	// @[<stdin>:1550:3]
  ...
  reg readDataValid;	// @[playground/src/Sram.scala:18:28]
  always @(posedge clock) begin	// @[<stdin>:1551:11]
    if (reset)	// @[<stdin>:1551:11]
      readDataValid <= 1'h0;	// @[playground/src/Sram.scala:9:25, :18:28]
    else	// @[<stdin>:1551:11]
      readDataValid <= ~readDataValid & io_ar_valid;	// @[playground/src/Sram.scala:18:28, :19:{17,32}]
  end // always @(posedge)
  ...
  assign io_r_valid = readDataValid;	// @[<stdin>:1550:3, playground/src/Sram.scala:18:28]
  assign io_b_valid = io_aw_valid & io_w_valid;	// @[<stdin>:1550:3, playground/src/Sram.scala:33:19]
endmodule

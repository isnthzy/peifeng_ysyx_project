...
module EX_stage(	// @[<stdin>:1196:3]
  ...
  wire        _Csr_alu_io_wen;	// @[playground/src/EX_stage.scala:120:21]
  wire [31:0] _Csr_alu_io_out;	// @[playground/src/EX_stage.scala:120:21]
  wire [31:0] _Alu_io_result;	// @[playground/src/EX_stage.scala:33:17]
  reg         ex_valid;	// @[playground/src/EX_stage.scala:23:33]
  wire        ex_ready_go = EX_ar_ready | EX_aw_ready & EX_w_ready;	// @[playground/src/EX_stage.scala:24:33, :25:{31,45}]
  wire        _EX_IO_ready_output = ~ex_valid | ex_ready_go & EX_to_ls_ready;	// @[playground/src/EX_stage.scala:23:33, :24:33, :26:{18,28,43}]
  wire        _EX_to_if_Br_B_taken_output = EX_IO_bits_b_taken & ex_valid;	// @[playground/src/EX_stage.scala:23:33, :40:42]
  wire        _EX_to_if_epc_target_T = EX_IO_bits_csr_cmd == 5'h3;	// @[playground/src/EX_stage.scala:54:39]
  wire        _EX_to_if_epc_target_T_1 = EX_IO_bits_csr_cmd == 5'h4;	// @[playground/src/EX_stage.scala:55:39]
  wire        st_wen = |EX_IO_bits_ld_type;	// @[playground/src/EX_stage.scala:58:37, :69:28]
  wire        ld_wen = |EX_IO_bits_st_type;	// @[playground/src/EX_stage.scala:68:28, :70:30]
  reg         DoAddrReadReg;	// @[playground/src/EX_stage.scala:73:28]
  reg         DoAddrWriteReg;	// @[playground/src/EX_stage.scala:74:29]
  wire        _EX_w_valid_output = DoAddrWriteReg;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:74:29]
  reg         DoWdataReg;	// @[playground/src/EX_stage.scala:75:25]
  wire        _EX_aw_valid_output = DoWdataReg;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:75:25]
  wire        _EX_ar_valid_output = DoAddrReadReg & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :73:28, :98:29]
  wire        _EX_to_id_csr_wen_output = _Csr_alu_io_wen & ex_valid;	// @[playground/src/EX_stage.scala:23:33, :120:21, :125:35]
  wire        _EX_to_id_csr_ecpt_wen_output = _EX_to_if_epc_target_T_1 & ex_valid;	// @[playground/src/EX_stage.scala:23:33, :55:39, :127:58]
  wire        _EX_to_ls_bits_dpic_bundle_ex_is_jal_T = EX_IO_bits_br_type == 4'h7;	// @[playground/src/EX_stage.scala:157:62]
  wire        _GEN = st_wen & ex_valid;	// @[playground/src/EX_stage.scala:23:33, :69:28, :84:15]
  always @(posedge clock) begin	// @[<stdin>:1197:11]
    if (reset) begin	// @[<stdin>:1197:11]
      ex_valid <= 1'h0;	// @[playground/src/EX_stage.scala:23:33]
      DoAddrReadReg <= 1'h0;	// @[playground/src/EX_stage.scala:23:33, :73:28]
      DoAddrWriteReg <= 1'h0;	// @[playground/src/EX_stage.scala:23:33, :74:29]
      DoWdataReg <= 1'h0;	// @[playground/src/EX_stage.scala:23:33, :75:25]
    end
    else begin	// @[<stdin>:1197:11]
      if (_EX_IO_ready_output)	// @[playground/src/EX_stage.scala:26:28]
        ex_valid <= EX_IO_valid;	// @[playground/src/EX_stage.scala:23:33]
      DoAddrReadReg <=
        ~(EX_ar_ready & _EX_ar_valid_output & ex_valid)
        & (ld_wen & ex_valid | DoAddrReadReg);	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :68:28, :73:28, :76:{15,27}, :77:18, :80:{19,31}, :81:18]
      DoAddrWriteReg <=
        ~(EX_aw_ready & _EX_aw_valid_output & ex_valid) & (_GEN | DoAddrWriteReg);	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :74:29, :84:{15,27}, :85:19, :89:{19,31}, :90:19]
      DoWdataReg <= ~(EX_w_ready & _EX_w_valid_output & ex_valid) & (_GEN | DoWdataReg);	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :75:25, :84:{15,27}, :86:15, :93:{18,30}, :94:15]
    end
  end // always @(posedge)
  Alu Alu (	// @[playground/src/EX_stage.scala:33:17]
    .io_op     (EX_IO_bits_alu_op),
    .io_src1   (EX_IO_bits_src1),
    .io_src2   (EX_IO_bits_src2),
    .io_result (_Alu_io_result)
  );
  Csr_alu Csr_alu (	// @[playground/src/EX_stage.scala:120:21]
    .io_csr_cmd   (EX_IO_bits_csr_cmd),
    .io_in_csr    (_Alu_io_result),	// @[playground/src/EX_stage.scala:33:17]
    .io_in_rdata1 (EX_IO_bits_rdata1),
    .io_wen       (_Csr_alu_io_wen),
    .io_out       (_Csr_alu_io_out)
  );
  assign EX_IO_ready = _EX_IO_ready_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:26:28]
  assign EX_to_ls_valid = ex_valid & ex_ready_go;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :24:33, :30:28]
  assign EX_to_ls_bits_csr_commit_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:125:35]
  assign EX_to_ls_bits_csr_commit_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1196:3]
  assign EX_to_ls_bits_csr_commit_wdata = _Csr_alu_io_out;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:120:21]
  assign EX_to_ls_bits_csr_commit_exception_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:127:58]
  ...
  assign EX_to_ls_bits_dpic_bundle_ex_func_flag =
    _EX_to_ls_bits_dpic_bundle_ex_is_jal_T | EX_IO_bits_br_type == 4'h8;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:157:{62,72,92}]
  assign EX_to_ls_bits_dpic_bundle_ex_is_jal = _EX_to_ls_bits_dpic_bundle_ex_is_jal_T;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:157:62]
  assign EX_to_ls_bits_dpic_bundle_ex_is_ret = EX_IO_bits_inst == 32'h8067;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:159:55]
  assign EX_to_ls_bits_dpic_bundle_ex_is_rd0 = EX_IO_bits_rd == 5'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:62:24, :160:53]
  ...
  assign EX_to_ls_bits_result = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:33:17]
  ...
  assign EX_to_id_fw_addr = ex_valid & EX_IO_bits_rf_wen ? EX_IO_bits_rd : 5'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :62:{24,34}]
  assign EX_to_id_fw_data = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:33:17]
  assign EX_to_id_csr_ecpt_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:127:58]
  assign EX_to_id_csr_ecpt_pc_wb = EX_IO_bits_pc;	// @[<stdin>:1196:3]
  assign EX_to_id_csr_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1196:3]
  assign EX_to_id_csr_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:125:35]
  assign EX_to_id_csr_wdata = _Csr_alu_io_out;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:120:21]
  assign EX_to_id_clog = (|EX_IO_bits_ld_type) & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :58:{37,44}]
  assign EX_to_if_epc_target =
    _EX_to_if_epc_target_T
      ? EX_IO_bits_csr_global_mepc
      : _EX_to_if_epc_target_T_1 ? EX_IO_bits_csr_global_mtvec : 32'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:41:58, :54:39, :55:39, :132:27, :133:28]
  assign EX_to_if_epc_taken = EX_IO_bits_pc_sel & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :131:51]
  assign EX_to_if_Br_B_taken = _EX_to_if_Br_B_taken_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:40:42]
  assign EX_to_if_Br_B_target =
    ...
    | EX_IO_bits_br_type == 4'h3 | EX_IO_bits_br_type == 4'h2 | EX_IO_bits_br_type == 4'h1
      ? _Alu_io_result
      : 32'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:33:17, :41:58]
  assign EX_to_if_flush =
    (_EX_to_if_Br_B_taken_output | _EX_to_if_epc_target_T | _EX_to_if_epc_target_T_1)
    & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:23:33, :40:42, :54:39, :55:{17,39,53}]
  assign EX_ar_valid = _EX_ar_valid_output;	// @[<stdin>:1196:3]
  assign EX_ar_bits_addr = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:33:17]
  assign EX_ar_bits_prot = 3'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:99:18]
  assign EX_aw_valid = _EX_aw_valid_output;	// @[<stdin>:1196:3]
  assign EX_aw_bits_addr = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:33:17]
  assign EX_aw_bits_prot = 3'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:99:18]
  assign EX_w_valid = _EX_w_valid_output;	// @[<stdin>:1196:3]
  assign EX_w_bits_data = EX_IO_bits_rdata2;	// @[<stdin>:1196:3]
  assign EX_w_bits_strb = {4'h0, EX_IO_bits_st_type};	// @[<stdin>:1196:3, playground/src/EX_stage.scala:70:30, :103:17]
  assign EX_b_ready = 1'h1;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:25:19]
endmodule

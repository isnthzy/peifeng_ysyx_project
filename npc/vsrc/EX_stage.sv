...
module EX_stage(	// @[<stdin>:1246:3]
  input         clock,	// @[<stdin>:1247:11]
                reset,	// @[<stdin>:1248:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1247:11]
    if (reset)	// @[<stdin>:1247:11]
  ...
  assign EX_IO_ready = _EX_IO_ready_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:30:28]
  assign EX_to_ls_valid = ex_valid & ex_ready_go;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:26:33, :27:33, :34:28]
  assign EX_to_ls_bits_csr_commit_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:106:35]
  assign EX_to_ls_bits_csr_commit_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_csr_commit_wdata = _Csr_alu_io_out;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:101:21]
  assign EX_to_ls_bits_csr_commit_exception_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:108:58]
  assign EX_to_ls_bits_csr_commit_exception_pc_wb = EX_IO_bits_pc;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_dpic_bundle_id_inv_flag = EX_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_dpic_bundle_ex_func_flag =
    _EX_to_ls_bits_dpic_bundle_ex_is_jal_T | EX_IO_bits_br_type == 4'h8;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:137:{62,72,92}]
  assign EX_to_ls_bits_dpic_bundle_ex_is_jal = _EX_to_ls_bits_dpic_bundle_ex_is_jal_T;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:137:62]
  assign EX_to_ls_bits_dpic_bundle_ex_is_ret = EX_IO_bits_inst == 32'h8067;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:139:55]
  assign EX_to_ls_bits_dpic_bundle_ex_is_rd0 = EX_IO_bits_rd == 5'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:74:24, :140:53]
  assign EX_to_ls_bits_ld_wen = ld_wen;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:23:28]
  assign EX_to_ls_bits_ld_type = EX_IO_bits_ld_type;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_csr_cmd = EX_IO_bits_csr_cmd;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_wb_sel = EX_IO_bits_wb_sel;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_rf_wen = EX_IO_bits_rf_wen;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_rd = EX_IO_bits_rd;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_result = _Alu_io_result;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:37:17]
  assign EX_to_ls_bits_nextpc = EX_IO_bits_nextpc;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_pc = EX_IO_bits_pc;	// @[<stdin>:1246:3]
  assign EX_to_ls_bits_inst = EX_IO_bits_inst;	// @[<stdin>:1246:3]
  assign EX_to_id_fw_addr = ex_valid & EX_IO_bits_rf_wen ? EX_IO_bits_rd : 5'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:26:33, :74:{24,34}]
  assign EX_to_id_fw_data = _Alu_io_result;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:37:17]
  assign EX_to_id_csr_ecpt_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:108:58]
  assign EX_to_id_csr_ecpt_pc_wb = EX_IO_bits_pc;	// @[<stdin>:1246:3]
  assign EX_to_id_csr_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1246:3]
  assign EX_to_id_csr_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:106:35]
  assign EX_to_id_csr_wdata = _Csr_alu_io_out;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:101:21]
  assign EX_to_id_clog = (|EX_IO_bits_ld_type) & ex_valid;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:26:33, :70:{37,44}]
  assign EX_to_id_flush = to_flush;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:61:46]
  assign EX_to_if_flush = to_flush;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:61:46]
  assign EX_to_preif_epc_target =
    _EX_to_preif_epc_target_T
      ? EX_IO_bits_csr_global_mepc
      : _EX_to_preif_epc_target_T_1 ? EX_IO_bits_csr_global_mtvec : 32'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:45:61, :60:32, :61:32, :113:30, :114:28]
  assign EX_to_preif_epc_taken = EX_IO_bits_pc_sel & ex_valid;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:26:33, :112:54]
  assign EX_to_preif_Br_B_taken = _EX_to_preif_Br_B_taken_output;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:44:45]
  assign EX_to_preif_Br_B_target =
    ...
    | EX_IO_bits_br_type == 4'h3 | EX_IO_bits_br_type == 4'h2 | EX_IO_bits_br_type == 4'h1
      ? _Alu_io_result
      : 32'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:37:17, :45:61]
  assign EX_to_preif_flush = to_flush;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:61:46]
  assign EX_ar_valid = ld_wen & ex_valid;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:23:28, :26:33, :84:22]
  assign EX_ar_bits_addr = _Alu_io_result;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:37:17]
  assign EX_ar_bits_prot = 3'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:86:18]
  assign EX_aw_valid = _EX_aw_valid_T;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:88:21]
  assign EX_aw_bits_addr = _Alu_io_result;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:37:17]
  assign EX_aw_bits_prot = 3'h0;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:86:18]
  assign EX_w_valid = _EX_aw_valid_T;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:88:21]
  assign EX_w_bits_data = EX_IO_bits_rdata2;	// @[<stdin>:1246:3]
  assign EX_w_bits_strb = {4'h0, EX_IO_bits_st_type};	// @[<stdin>:1246:3, playground/src/EX_stage.scala:81:30, :90:17]
  assign EX_b_ready = ex_valid;	// @[<stdin>:1246:3, playground/src/EX_stage.scala:26:33]
endmodule

...
module EX_stage(	// @[<stdin>:1081:3]
  input         clock,	// @[<stdin>:1082:11]
                reset,	// @[<stdin>:1083:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1082:11]
    if (reset)	// @[<stdin>:1082:11]
  ...
  assign EX_IO_ready = _EX_IO_ready_output;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:19:28]
  assign EX_to_ls_valid = ex_valid & ex_ready_go;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:16:33, :17:33, :23:28]
  assign EX_to_ls_bits_dpic_bundle_id_inv_flag = EX_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_dpic_bundle_ex_func_flag =
    _EX_to_ls_bits_dpic_bundle_ex_is_jal_T | EX_IO_bits_br_type == 4'h8;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:75:{62,72,92}]
  assign EX_to_ls_bits_dpic_bundle_ex_is_jal = _EX_to_ls_bits_dpic_bundle_ex_is_jal_T;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:75:62]
  assign EX_to_ls_bits_dpic_bundle_ex_is_ret = EX_IO_bits_inst == 32'h8067;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:77:55]
  assign EX_to_ls_bits_dpic_bundle_ex_is_rd0 = EX_IO_bits_rd == 5'h0;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:49:25, :78:53]
  assign EX_to_ls_bits_pc_sel = EX_IO_bits_pc_sel;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_csr_addr = EX_IO_bits_csr_addr;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_csr_cmd = EX_IO_bits_csr_cmd;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_st_type = EX_IO_bits_st_type;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_ld_type = EX_IO_bits_ld_type;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_ebreak_flag = EX_IO_bits_ebreak_flag;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_wb_sel = EX_IO_bits_wb_sel;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_wen = EX_IO_bits_wen;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_rd = EX_IO_bits_rd;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_rdata2 = EX_IO_bits_rdata2;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_result = _Alu_io_result;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:26:17]
  assign EX_to_ls_bits_nextpc = EX_IO_bits_nextpc;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_pc = EX_IO_bits_pc;	// @[<stdin>:1081:3]
  assign EX_to_ls_bits_inst = EX_IO_bits_inst;	// @[<stdin>:1081:3]
  assign EX_bypass_id_addr = ex_valid & EX_IO_bits_wen ? EX_IO_bits_rd : 5'h0;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:16:33, :49:{25,35}]
  assign EX_bypass_id_data = _Alu_io_result;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:26:17]
  assign EX_clog_id = (|EX_IO_bits_ld_type) & ex_valid;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:16:33, :45:{34,41}]
  assign EX_flush_out = _EX_br_bus_taken_output & ex_valid;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:16:33, :37:31, :42:32]
  assign EX_br_bus_taken = _EX_br_bus_taken_output;	// @[<stdin>:1081:3, playground/src/EX_stage.scala:37:31]
endmodule

...
module EX_stage(	// @[<stdin>:1196:3]
  ...
  wire        _Csr_alu_io_wen;	// @[playground/src/EX_stage.scala:116:21]
  wire [31:0] _Csr_alu_io_out;	// @[playground/src/EX_stage.scala:116:21]
  wire [31:0] _Alu_io_result;	// @[playground/src/EX_stage.scala:29:17]
  wire        ex_ready_go = 1'h1;	// @[playground/src/EX_stage.scala:6:12, :20:33, :21:31]
  ...
  wire        _EX_to_if_Br_B_taken_output = EX_IO_bits_b_taken & ex_valid;	// @[playground/src/EX_stage.scala:19:33, :36:42]
  wire        _EX_to_if_epc_target_T = EX_IO_bits_csr_cmd == 5'h3;	// @[playground/src/EX_stage.scala:50:39]
  wire        _EX_to_if_epc_target_T_1 = EX_IO_bits_csr_cmd == 5'h4;	// @[playground/src/EX_stage.scala:51:39]
  wire        st_wen = |EX_IO_bits_ld_type;	// @[playground/src/EX_stage.scala:54:37, :65:28]
  wire        ld_wen = |EX_IO_bits_st_type;	// @[playground/src/EX_stage.scala:64:28, :66:30]
  reg         DoAddrReadReg;	// @[playground/src/EX_stage.scala:69:28]
  reg         DoAddrWriteReg;	// @[playground/src/EX_stage.scala:70:29]
  reg         DoWdataReg;	// @[playground/src/EX_stage.scala:71:25]
  wire        _EX_ar_valid_output = DoAddrReadReg & ex_valid;	// @[playground/src/EX_stage.scala:19:33, :69:28, :94:29]
  wire        _EX_to_id_csr_wen_output = _Csr_alu_io_wen & ex_valid;	// @[playground/src/EX_stage.scala:19:33, :116:21, :121:35]
  wire        _EX_to_id_csr_ecpt_wen_output = _EX_to_if_epc_target_T_1 & ex_valid;	// @[playground/src/EX_stage.scala:19:33, :51:39, :123:58]
  wire        _EX_to_ls_bits_dpic_bundle_ex_is_jal_T = EX_IO_bits_br_type == 4'h7;	// @[playground/src/EX_stage.scala:153:62]
  wire        _GEN = st_wen & ex_valid;	// @[playground/src/EX_stage.scala:19:33, :65:28, :80:15]
  always @(posedge clock) begin	// @[<stdin>:1197:11]
    if (reset) begin	// @[<stdin>:1197:11]
      ex_valid <= 1'h0;	// @[playground/src/EX_stage.scala:19:33]
      DoAddrReadReg <= 1'h0;	// @[playground/src/EX_stage.scala:19:33, :69:28]
      DoAddrWriteReg <= 1'h0;	// @[playground/src/EX_stage.scala:19:33, :70:29]
      DoWdataReg <= 1'h0;	// @[playground/src/EX_stage.scala:19:33, :71:25]
    end
    else begin	// @[<stdin>:1197:11]
      if (_EX_IO_ready_output)	// @[playground/src/EX_stage.scala:22:28]
        ex_valid <= EX_IO_valid;	// @[playground/src/EX_stage.scala:19:33]
      DoAddrReadReg <=
        ~(_EX_ar_valid_output & ex_valid) & (ld_wen & ex_valid | DoAddrReadReg);	// @[playground/src/EX_stage.scala:19:33, :64:28, :69:28, :72:{15,27}, :73:18, :76:{19,31}, :77:18, :94:29]
      DoAddrWriteReg <= ~(DoWdataReg & ex_valid) & (_GEN | DoAddrWriteReg);	// @[playground/src/EX_stage.scala:19:33, :70:29, :71:25, :80:{15,27}, :81:19, :85:{19,31}, :86:19]
      DoWdataReg <= ~(DoAddrWriteReg & ex_valid) & (_GEN | DoWdataReg);	// @[playground/src/EX_stage.scala:19:33, :70:29, :71:25, :80:{15,27}, :82:15, :89:{18,30}, :90:15]
    end
  end // always @(posedge)
  ...
  Csr_alu Csr_alu (	// @[playground/src/EX_stage.scala:116:21]
  ...
  assign EX_IO_ready = _EX_IO_ready_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:22:28]
  assign EX_to_ls_valid = ex_valid & ex_ready_go;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:19:33, :20:33, :26:28]
  assign EX_to_ls_bits_csr_commit_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:121:35]
  assign EX_to_ls_bits_csr_commit_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1196:3]
  assign EX_to_ls_bits_csr_commit_wdata = _Csr_alu_io_out;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:116:21]
  assign EX_to_ls_bits_csr_commit_exception_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:123:58]
  ...
  assign EX_to_ls_bits_dpic_bundle_ex_func_flag =
    _EX_to_ls_bits_dpic_bundle_ex_is_jal_T | EX_IO_bits_br_type == 4'h8;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:153:{62,72,92}]
  assign EX_to_ls_bits_dpic_bundle_ex_is_jal = _EX_to_ls_bits_dpic_bundle_ex_is_jal_T;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:153:62]
  assign EX_to_ls_bits_dpic_bundle_ex_is_ret = EX_IO_bits_inst == 32'h8067;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:155:55]
  assign EX_to_ls_bits_dpic_bundle_ex_is_rd0 = EX_IO_bits_rd == 5'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:58:24, :156:53]
  ...
  assign EX_to_ls_bits_result = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:29:17]
  ...
  assign EX_to_id_fw_addr = ex_valid & EX_IO_bits_rf_wen ? EX_IO_bits_rd : 5'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:19:33, :58:{24,34}]
  assign EX_to_id_fw_data = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:29:17]
  assign EX_to_id_csr_ecpt_wen = _EX_to_id_csr_ecpt_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:123:58]
  assign EX_to_id_csr_ecpt_pc_wb = EX_IO_bits_pc;	// @[<stdin>:1196:3]
  assign EX_to_id_csr_waddr = EX_IO_bits_csr_addr;	// @[<stdin>:1196:3]
  assign EX_to_id_csr_wen = _EX_to_id_csr_wen_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:121:35]
  assign EX_to_id_csr_wdata = _Csr_alu_io_out;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:116:21]
  assign EX_to_id_clog = (|EX_IO_bits_ld_type) & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:19:33, :54:{37,44}]
  assign EX_to_if_epc_target =
    _EX_to_if_epc_target_T
      ? EX_IO_bits_csr_global_mepc
      : _EX_to_if_epc_target_T_1 ? EX_IO_bits_csr_global_mtvec : 32'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:37:58, :50:39, :51:39, :128:27, :129:28]
  assign EX_to_if_epc_taken = EX_IO_bits_pc_sel & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:19:33, :127:51]
  assign EX_to_if_Br_B_taken = _EX_to_if_Br_B_taken_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:36:42]
  assign EX_to_if_Br_B_target =
    ...
    | EX_IO_bits_br_type == 4'h3 | EX_IO_bits_br_type == 4'h2 | EX_IO_bits_br_type == 4'h1
      ? _Alu_io_result
      : 32'h0;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:29:17, :37:58]
  assign EX_to_if_flush =
    (_EX_to_if_Br_B_taken_output | _EX_to_if_epc_target_T | _EX_to_if_epc_target_T_1)
    & ex_valid;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:19:33, :36:42, :50:39, :51:{17,39,53}]
  assign EX_ar_valid = _EX_ar_valid_output;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:94:29]
  assign EX_ar_bits_addr = _Alu_io_result;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:29:17]
  assign EX_aw_valid = DoWdataReg;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:71:25]
  assign EX_w_valid = DoAddrWriteReg;	// @[<stdin>:1196:3, playground/src/EX_stage.scala:70:29]
  assign EX_w_bits_data = EX_IO_bits_rdata2;	// @[<stdin>:1196:3]
  assign EX_w_bits_strb = {4'h0, EX_IO_bits_st_type};	// @[<stdin>:1196:3, playground/src/EX_stage.scala:66:30, :99:17]
endmodule

...
module EX_stage(	// @[<stdin>:850:3]
  input         clock,	// @[<stdin>:851:11]
                reset,	// @[<stdin>:852:11]
  ...
  dpi_func dpi_func (	// @[playground/src/EX_stage.scala:51:22]
    .clock     (clock),
    .reset     (reset),
    .func_flag (_dpi_func_io_is_jal_T | _dpi_func_io_func_flag_T_1),	// @[playground/src/EX_stage.scala:22:39, :23:39, :54:50]
    .is_jal    (_dpi_func_io_is_jal_T),	// @[playground/src/EX_stage.scala:22:39]
    .pc        (EX_IO_nextpc),
    .nextpc    (32'h0),	// @[playground/src/EX_stage.scala:30:47]
    .rd        ({27'h0, EX_IO_rd}),	// @[playground/src/EX_stage.scala:58:17]
    .inst      (EX_IO_inst)
  );
  assign EX_to_ls_ebreak_flag = EX_IO_ebreak_flag;	// @[<stdin>:850:3]
  assign EX_to_ls_wb_sel = EX_IO_wb_sel;	// @[<stdin>:850:3]
  assign EX_to_ls_wen = EX_IO_wen;	// @[<stdin>:850:3]
  assign EX_to_ls_rd = EX_IO_rd;	// @[<stdin>:850:3]
  assign EX_to_ls_result = _Alu_io_result;	// @[<stdin>:850:3, playground/src/EX_stage.scala:12:17]
  assign EX_to_ls_nextpc = EX_IO_nextpc;	// @[<stdin>:850:3]
  assign EX_to_ls_pc = EX_IO_pc;	// @[<stdin>:850:3]
  assign EX_br_bus_is_jump =
    ...
    & ~rs1_lt_rs2_s | EX_IO_br_type == 4'h4 & ~rs1_lt_rs2_u;	// @[<stdin>:850:3, playground/src/EX_stage.scala:18:33, :19:40, :20:34, :22:39, :23:39, :24:{39,49}, :25:{39,49,52}, :26:{39,49}, :27:{39,49}, :28:{39,49,52}, :29:{23,39,49,52}]
  assign EX_br_bus_dnpc =
    EX_IO_br_type == 4'h8
      ...
      : EX_IO_br_type == 4'h7 | EX_IO_br_type == 4'h6 | EX_IO_br_type == 4'h5
        ...
        | EX_IO_br_type == 4'h1
          ? _Alu_io_result
          : 32'h0;	// @[<stdin>:850:3, playground/src/EX_stage.scala:12:17, :22:39, :23:39, :24:39, :25:{39,52}, :26:39, :27:39, :28:39, :29:39, :30:47, :39:{18,32}]
endmodule

...
module EX_stage(	// @[<stdin>:954:3]
  input         clock,	// @[<stdin>:955:11]
                reset,	// @[<stdin>:956:11]
  ...
  wire        _EX_to_ls_bits_dpic_bundle_ex_is_jal_T = EX_IO_bits_br_type == 4'h7;	// @[playground/src/EX_stage.scala:33:44]
  wire        _EX_to_ls_bits_dpic_bundle_ex_func_flag_T_1 = EX_IO_bits_br_type == 4'h8;	// @[playground/src/EX_stage.scala:34:44]
  always @(posedge clock) begin	// @[<stdin>:955:11]
    if (reset)	// @[<stdin>:955:11]
  ...
  assign EX_IO_ready = _EX_IO_ready_output;	// @[<stdin>:954:3, playground/src/EX_stage.scala:16:28]
  assign EX_to_ls_valid = ex_valid & ex_ready_go;	// @[<stdin>:954:3, playground/src/EX_stage.scala:13:33, :14:33, :20:28]
  assign EX_to_ls_bits_dpic_bundle_id_inv_flag = EX_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_dpic_bundle_ex_func_flag =
    _EX_to_ls_bits_dpic_bundle_ex_is_jal_T | _EX_to_ls_bits_dpic_bundle_ex_func_flag_T_1;	// @[<stdin>:954:3, playground/src/EX_stage.scala:33:44, :34:44, :72:72]
  assign EX_to_ls_bits_dpic_bundle_ex_is_jal = _EX_to_ls_bits_dpic_bundle_ex_is_jal_T;	// @[<stdin>:954:3, playground/src/EX_stage.scala:33:44]
  assign EX_to_ls_bits_dpic_bundle_ex_is_ret = EX_IO_bits_inst == 32'h8067;	// @[<stdin>:954:3, playground/src/EX_stage.scala:74:55]
  assign EX_to_ls_bits_dpic_bundle_ex_is_rd0 = EX_IO_bits_rd == 5'h0;	// @[<stdin>:954:3, playground/src/EX_stage.scala:75:53]
  assign EX_to_ls_bits_pc_sel = EX_IO_bits_pc_sel;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_csr_addr = EX_IO_bits_csr_addr;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_csr_cmd = EX_IO_bits_csr_cmd;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_st_type = EX_IO_bits_st_type;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_ld_type = EX_IO_bits_ld_type;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_ebreak_flag = EX_IO_bits_ebreak_flag;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_wb_sel = EX_IO_bits_wb_sel;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_wen = EX_IO_bits_wen;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_rd = EX_IO_bits_rd;	// @[<stdin>:954:3]
  assign EX_to_ls_bits_rdata2 = EX_IO_bits_rdata2;	// @[<stdin>:954:3]
  ...
  assign EX_br_bus_is_jump =
    ...
    | EX_IO_bits_br_type == 4'h4 & ~rs1_lt_rs2_u;	// @[<stdin>:954:3, playground/src/EX_stage.scala:29:40, :30:47, :31:41, :33:44, :34:44, :35:{45,55}, :36:{45,55,58}, :37:{45,55}, :38:{45,55}, :39:{45,55,58}, :40:{23,45,55,58}]
    ...
    EX_IO_bits_br_type == 4'h8
      ...
      : EX_IO_bits_br_type == 4'h7 | EX_IO_bits_br_type == 4'h6
        ...
        | EX_IO_bits_br_type == 4'h1
          ? _Alu_io_result
          : 32'h0;	// @[<stdin>:954:3, playground/src/EX_stage.scala:13:33, :23:17, :33:44, :34:44, :35:45, :36:45, :37:45, :38:45, :39:45, :40:45, :41:52, :50:{18,32}]
endmodule

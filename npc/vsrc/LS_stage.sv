...
module LS_stage(	// @[<stdin>:1116:3]
  input         clock,	// @[<stdin>:1117:11]
                reset,	// @[<stdin>:1118:11]
  ...
  reg         ls_valid;	// @[playground/src/LS_stage.scala:16:33]
  wire        ld_wen;	// @[playground/src/LS_stage.scala:13:28]
  wire        ls_ready_go = ~ld_wen;	// @[playground/src/LS_stage.scala:13:28, :16:33, :17:33, :18:19]
  wire        _LS_IO_ready_output = ~ls_valid | ls_ready_go & LS_to_wb_ready;	// @[playground/src/LS_stage.scala:16:33, :17:33, :19:{18,28,43}]
  assign ld_wen = (|LS_IO_bits_ld_type) & ls_valid;	// @[playground/src/LS_stage.scala:13:28, :16:33, :25:{37,44}]
  always_comb begin	// @[playground/src/LS_stage.scala:38:46]
    casez (LS_IO_bits_ld_type)	// @[playground/src/LS_stage.scala:38:46]
      3'b000:
        casez_tmp = 32'h0;	// @[playground/src/LS_stage.scala:38:46]
      3'b001:
        casez_tmp = _dpi_ls_rdata;	// @[playground/src/LS_stage.scala:28:20, :38:46]
      3'b010:
        casez_tmp = {{16{_dpi_ls_rdata[15]}}, _dpi_ls_rdata[15:0]};	// @[playground/src/Bundle.scala:115:{10,15,37}, playground/src/LS_stage.scala:28:20, :38:46, :40:34]
      3'b011:
        casez_tmp = {{24{_dpi_ls_rdata[7]}}, _dpi_ls_rdata[7:0]};	// @[playground/src/Bundle.scala:115:{10,15,37}, playground/src/LS_stage.scala:28:20, :38:46, :41:34]
      3'b100:
        casez_tmp = {16'h0, _dpi_ls_rdata[15:0]};	// @[playground/src/Bundle.scala:115:15, :126:10, playground/src/LS_stage.scala:28:20, :38:46, :40:34]
      3'b101:
        casez_tmp = {24'h0, _dpi_ls_rdata[7:0]};	// @[playground/src/Bundle.scala:115:15, :126:10, playground/src/LS_stage.scala:28:20, :38:46, :41:34]
      3'b110:
        casez_tmp = 32'h0;	// @[playground/src/LS_stage.scala:38:46]
      default:
        casez_tmp = 32'h0;	// @[playground/src/LS_stage.scala:38:46]
    endcase	// @[playground/src/LS_stage.scala:38:46]
  end // always_comb
  wire [31:0] ram_data = casez_tmp;	// @[playground/src/LS_stage.scala:27:30, :38:46]
  always_comb begin	// @[playground/src/LS_stage.scala:54:57]
    casez (LS_IO_bits_wb_sel)	// @[playground/src/LS_stage.scala:54:57]
      ...
      2'b01:
        casez_tmp_0 = ram_data;	// @[playground/src/LS_stage.scala:27:30, :54:57]
      2'b10:
  ...
  always @(posedge clock) begin	// @[<stdin>:1117:11]
    if (reset)	// @[<stdin>:1117:11]
      ls_valid <= 1'h0;	// @[playground/src/LS_stage.scala:16:33]
    else if (_LS_IO_ready_output)	// @[playground/src/LS_stage.scala:19:28]
      ls_valid <= LS_IO_valid;	// @[playground/src/LS_stage.scala:16:33]
  end // always @(posedge)
  dpi_ls dpi_ls (	// @[playground/src/LS_stage.scala:28:20]
    .clock  (clock),
    .reset  (reset),
    .ld_wen (ld_wen),	// @[playground/src/LS_stage.scala:13:28]
    .st_wen ((|LS_IO_bits_st_type) & ls_valid),	// @[playground/src/LS_stage.scala:16:33, :32:{47,54}]
  ...
  assign LS_IO_ready = _LS_IO_ready_output;	// @[<stdin>:1116:3, playground/src/LS_stage.scala:19:28]
  assign LS_bypass_id_addr = ls_valid & LS_IO_bits_wen ? LS_IO_bits_rd : 5'h0;	// @[<stdin>:1116:3, playground/src/LS_stage.scala:16:33, :65:{25,35}]
  assign LS_bypass_id_data = casez_tmp_0;	// @[<stdin>:1116:3, playground/src/LS_stage.scala:54:57]
  assign LS_to_wb_valid = ls_valid & ls_ready_go;	// @[<stdin>:1116:3, playground/src/LS_stage.scala:16:33, :17:33, :23:28]
  assign LS_to_wb_bits_dpic_bundle_id_inv_flag = LS_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_dpic_bundle_ex_func_flag = LS_IO_bits_dpic_bundle_ex_func_flag;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_jal = LS_IO_bits_dpic_bundle_ex_is_jal;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_ret = LS_IO_bits_dpic_bundle_ex_is_ret;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_rd0 = LS_IO_bits_dpic_bundle_ex_is_rd0;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_pc_sel = LS_IO_bits_pc_sel;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_csr_addr = LS_IO_bits_csr_addr;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_csr_cmd = LS_IO_bits_csr_cmd;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_ebreak_flag = LS_IO_bits_ebreak_flag;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_wen = LS_IO_bits_wen;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_rd = LS_IO_bits_rd;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_result = casez_tmp_0;	// @[<stdin>:1116:3, playground/src/LS_stage.scala:54:57]
  assign LS_to_wb_bits_nextpc = LS_IO_bits_nextpc;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_pc = LS_IO_bits_pc;	// @[<stdin>:1116:3]
  assign LS_to_wb_bits_inst = LS_IO_bits_inst;	// @[<stdin>:1116:3]
endmodule

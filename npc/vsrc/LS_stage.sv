...
module LS_stage(	// @[<stdin>:1122:3]
  ...
  always @(posedge clock) begin	// @[<stdin>:1123:11]
    if (reset)	// @[<stdin>:1123:11]
  ...
  assign LS_IO_ready = _LS_IO_ready_output;	// @[<stdin>:1122:3, playground/src/LS_stage.scala:16:28]
  assign LS_bypass_id_addr = ls_valid & LS_IO_bits_wen ? LS_IO_bits_rd : 5'h0;	// @[<stdin>:1122:3, playground/src/LS_stage.scala:13:33, :61:{25,35}]
  assign LS_bypass_id_data = casez_tmp_0;	// @[<stdin>:1122:3, playground/src/LS_stage.scala:50:57]
  assign LS_to_wb_valid = ls_valid & ls_ready_go;	// @[<stdin>:1122:3, playground/src/LS_stage.scala:13:33, :14:33, :20:28]
  assign LS_to_wb_bits_dpic_bundle_id_inv_flag = LS_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_dpic_bundle_ex_func_flag = LS_IO_bits_dpic_bundle_ex_func_flag;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_jal = LS_IO_bits_dpic_bundle_ex_is_jal;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_ret = LS_IO_bits_dpic_bundle_ex_is_ret;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_rd0 = LS_IO_bits_dpic_bundle_ex_is_rd0;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_pc_sel = LS_IO_bits_pc_sel;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_csr_addr = LS_IO_bits_csr_addr;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_csr_cmd = LS_IO_bits_csr_cmd;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_ebreak_flag = LS_IO_bits_ebreak_flag;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_wen = LS_IO_bits_wen;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_rd = LS_IO_bits_rd;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_result = casez_tmp_0;	// @[<stdin>:1122:3, playground/src/LS_stage.scala:50:57]
  assign LS_to_wb_bits_nextpc = LS_IO_bits_nextpc;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_pc = LS_IO_bits_pc;	// @[<stdin>:1122:3]
  assign LS_to_wb_bits_inst = LS_IO_bits_inst;	// @[<stdin>:1122:3]
endmodule

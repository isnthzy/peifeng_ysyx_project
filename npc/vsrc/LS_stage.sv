...
module LS_stage(	// @[<stdin>:1326:3]
  input         clock,	// @[<stdin>:1327:11]
                reset,	// @[<stdin>:1328:11]
  ...
  wire        _LS_r_ready_output = ls_valid;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:18:33]
  ...
  wire [31:0] data_sram_rdata = _LS_r_ready_output & LS_r_valid ? LS_r_bits_data : 32'h0;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:15:44, :29:18, :30:20, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  ...
  always @(posedge clock) begin	// @[<stdin>:1327:11]
    if (reset)	// @[<stdin>:1327:11]
  ...
  assign LS_IO_ready = _LS_IO_ready_output;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:21:28]
  assign LS_to_wb_valid = ls_valid & ls_ready_go;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:18:33, :19:33, :25:28]
  assign LS_to_wb_bits_csr_commit_wen = LS_IO_bits_csr_commit_wen;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_commit_waddr = LS_IO_bits_csr_commit_waddr;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_commit_wdata = LS_IO_bits_csr_commit_wdata;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_commit_exception_wen = LS_IO_bits_csr_commit_exception_wen;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_commit_exception_mcause_in =
    LS_IO_bits_csr_commit_exception_mcause_in;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_commit_exception_pc_wb = LS_IO_bits_csr_commit_exception_pc_wb;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_dpic_bundle_id_inv_flag = LS_IO_bits_dpic_bundle_id_inv_flag;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_dpic_bundle_ex_func_flag = LS_IO_bits_dpic_bundle_ex_func_flag;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_jal = LS_IO_bits_dpic_bundle_ex_is_jal;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_ret = LS_IO_bits_dpic_bundle_ex_is_ret;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_dpic_bundle_ex_is_rd0 = LS_IO_bits_dpic_bundle_ex_is_rd0;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_csr_cmd = LS_IO_bits_csr_cmd;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_rf_wen = LS_IO_bits_rf_wen;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_rd = LS_IO_bits_rd;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_result = casez_tmp_0;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:51:57]
  assign LS_to_wb_bits_nextpc = LS_IO_bits_nextpc;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_pc = LS_IO_bits_pc;	// @[<stdin>:1326:3]
  assign LS_to_wb_bits_inst = LS_IO_bits_inst;	// @[<stdin>:1326:3]
  assign LS_to_id_fw_addr = ls_valid & LS_IO_bits_rf_wen ? LS_IO_bits_rd : 5'h0;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:18:33, :62:{24,34}]
  assign LS_to_id_fw_data = casez_tmp_0;	// @[<stdin>:1326:3, playground/src/LS_stage.scala:51:57]
  assign LS_r_ready = _LS_r_ready_output;	// @[<stdin>:1326:3]
endmodule

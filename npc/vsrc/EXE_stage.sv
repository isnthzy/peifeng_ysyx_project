...
module EXE_stage(	// @[<stdin>:592:3]
  input         clock,	// @[<stdin>:593:11]
                reset,	// @[<stdin>:594:11]
  ...
  assign io_result = _io_result_output;	// @[<stdin>:592:3, playground/src/EXE_stage.scala:34:17]
  assign io_jalr_taget = {_jalr_tmp_T[31:1], 1'h0};	// @[<stdin>:592:3, playground/src/EXE_stage.scala:17:25, :36:29, :37:{21,30}]
  assign io_sram_valid = io_d_ebus_sram_valid;	// @[<stdin>:592:3]
  assign io_sram_wen = io_d_ebus_sram_wen;	// @[<stdin>:592:3]
  assign io_sram_wdata = {27'h0, io_d_ebus_src2};	// @[<stdin>:592:3, playground/src/EXE_stage.scala:41:16]
  assign io_sram_wmask = io_d_ebus_wmask;	// @[<stdin>:592:3]
endmodule

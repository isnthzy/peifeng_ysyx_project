...
module EXE_stage(	// @[<stdin>:573:3]
  ...
  reg  [31:0] result;	// @[playground/src/EXE_stage.scala:33:17]
  wire [31:0] _jalr_tmp_T = _alu_io_result + io_d_ebus_imm;	// @[playground/src/EXE_stage.scala:23:18, :36:29]
  always @(posedge clock) begin	// @[<stdin>:574:11]
    if (io_d_ebus_result_is_imm)	// @[playground/src/EXE_stage.scala:4:12]
      result <= io_d_ebus_imm;	// @[playground/src/EXE_stage.scala:33:17]
    else if (io_d_ebus_result_is_snpc)	// @[playground/src/EXE_stage.scala:4:12]
      result <= io_d_ebus_snpc;	// @[playground/src/EXE_stage.scala:33:17]
    else	// @[playground/src/EXE_stage.scala:4:12]
      result <= _alu_io_result;	// @[playground/src/EXE_stage.scala:23:18, :33:17]
  end // always @(posedge)
  RegFile RegFile (	// @[playground/src/EXE_stage.scala:14:21]
    .clock     (clock),
    .reset     (reset),
    .io_waddr  (io_d_ebus_rd),
    .io_wdata  (result),	// @[playground/src/EXE_stage.scala:33:17]
  ...
  assign io_result = result;	// @[<stdin>:573:3, playground/src/EXE_stage.scala:33:17]

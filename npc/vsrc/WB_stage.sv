...
module WB_stage(	// @[<stdin>:1359:3]
  input         clock,	// @[<stdin>:1360:11]
                reset,	// @[<stdin>:1361:11]
  ...
  wire        _WB_to_id_wen_output = WB_IO_bits_wen & wb_valid;	// @[playground/src/WB_stage.scala:17:33, :38:33]
  always @(posedge clock) begin	// @[<stdin>:1360:11]
    if (reset)	// @[<stdin>:1360:11]
  ...
  DPI_stage DPI_stage (	// @[playground/src/WB_stage.scala:45:23]
    ...
    .DPI_ret_reg_data (_WB_to_id_wdata_output[0])	// @[playground/src/WB_stage.scala:36:22, :56:29]
  );
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1359:3, playground/src/WB_stage.scala:20:28]
  assign WB_to_id_waddr = WB_IO_bits_rd;	// @[<stdin>:1359:3]
  assign WB_to_id_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1359:3, playground/src/WB_stage.scala:36:22]
  assign WB_to_id_wen = _WB_to_id_wen_output;	// @[<stdin>:1359:3, playground/src/WB_stage.scala:38:33]
  assign WB_to_if_epc_wen = WB_IO_bits_pc_sel;	// @[<stdin>:1359:3]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1359:3]
  assign WB_debug_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1359:3, playground/src/WB_stage.scala:36:22]
  assign WB_debug_wen = _WB_to_id_wen_output;	// @[<stdin>:1359:3, playground/src/WB_stage.scala:38:33]
endmodule

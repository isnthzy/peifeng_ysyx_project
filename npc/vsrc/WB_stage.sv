...
module WB_stage(	// @[<stdin>:1448:3]
  input         clock,	// @[<stdin>:1449:11]
                reset,	// @[<stdin>:1450:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1449:11]
    if (reset)	// @[<stdin>:1449:11]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1448:3, playground/src/WB_stage.scala:19:28]
  assign WB_to_rf_waddr = WB_IO_bits_rd;	// @[<stdin>:1448:3]
  assign WB_to_rf_wdata = WB_IO_bits_result;	// @[<stdin>:1448:3]
  assign WB_to_rf_wen = _WB_to_rf_wen_output;	// @[<stdin>:1448:3, playground/src/WB_stage.scala:28:33]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1448:3]
  assign WB_debug_wdata = WB_IO_bits_result;	// @[<stdin>:1448:3]
  assign WB_debug_wen = _WB_to_rf_wen_output;	// @[<stdin>:1448:3, playground/src/WB_stage.scala:28:33]
endmodule

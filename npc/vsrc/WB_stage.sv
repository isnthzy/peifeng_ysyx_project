...
module WB_stage(	// @[<stdin>:1258:3]
  input         clock,	// @[<stdin>:1259:11]
                reset,	// @[<stdin>:1260:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1259:11]
    if (reset)	// @[<stdin>:1259:11]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1258:3, playground/src/WB_stage.scala:20:28]
  assign WB_to_id_waddr = WB_IO_bits_rd;	// @[<stdin>:1258:3]
  assign WB_to_id_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1258:3, playground/src/WB_stage.scala:36:22]
  assign WB_to_id_wen = WB_IO_bits_wen;	// @[<stdin>:1258:3]
  assign WB_to_if_epc_wen = WB_IO_bits_pc_sel;	// @[<stdin>:1258:3]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1258:3]
  assign WB_debug_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1258:3, playground/src/WB_stage.scala:36:22]
  assign WB_debug_wen = WB_IO_bits_wen;	// @[<stdin>:1258:3]
endmodule

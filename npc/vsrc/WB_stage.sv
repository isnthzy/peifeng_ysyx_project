...
module WB_stage(	// @[<stdin>:1553:3]
  ...
  wire _WB_IO_ready_output = (~wb_valid | wb_ready_go) & ~reset;	// @[playground/src/WB_stage.scala:17:33, :18:33, :20:{19,29,45,48}]
  ...
  always @(posedge clock) begin	// @[<stdin>:1554:11]
    if (reset)	// @[<stdin>:1554:11]
      wb_valid <= 1'h0;	// @[playground/src/WB_stage.scala:17:33]
    else if (_WB_IO_ready_output)	// @[playground/src/WB_stage.scala:20:45]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1553:3, playground/src/WB_stage.scala:20:45]
  assign WB_to_id_rf_waddr = WB_IO_bits_rd;	// @[<stdin>:1553:3]
  assign WB_to_id_rf_wdata = WB_IO_bits_result;	// @[<stdin>:1553:3]
  assign WB_to_id_rf_wen = _WB_to_id_rf_wen_output;	// @[<stdin>:1553:3, playground/src/WB_stage.scala:28:39]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1553:3]
  assign WB_debug_wdata = WB_IO_bits_result;	// @[<stdin>:1553:3]
  assign WB_debug_wen = _WB_to_id_rf_wen_output;	// @[<stdin>:1553:3, playground/src/WB_stage.scala:28:39]
endmodule

...
module WB_stage(	// @[<stdin>:1317:3]
  ...
  DPI_stage DPI_stage (	// @[playground/src/WB_stage.scala:45:23]
    ...
    .DPI_ret_reg_data (_WB_to_id_wdata_output[0])	// @[playground/src/WB_stage.scala:36:22, :55:29]
  );
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1317:3, playground/src/WB_stage.scala:20:28]
  assign WB_to_id_waddr = WB_IO_bits_rd;	// @[<stdin>:1317:3]
  assign WB_to_id_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1317:3, playground/src/WB_stage.scala:36:22]
  assign WB_to_id_wen = WB_IO_bits_wen;	// @[<stdin>:1317:3]
  assign WB_to_if_epc_wen = WB_IO_bits_pc_sel;	// @[<stdin>:1317:3]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1317:3]
  assign WB_debug_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1317:3, playground/src/WB_stage.scala:36:22]
  assign WB_debug_wen = WB_IO_bits_wen;	// @[<stdin>:1317:3]
endmodule

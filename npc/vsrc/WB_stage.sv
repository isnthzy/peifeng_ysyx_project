...
module WB_stage(	// @[<stdin>:1446:3]
  input         clock,	// @[<stdin>:1447:11]
                reset,	// @[<stdin>:1448:11]
                ...
                WB_IO_bits_ebreak_flag,	// @[playground/src/WB_stage.scala:7:12]
  ...
  wire wb_ready_go = 1'h1;	// @[playground/src/WB_stage.scala:17:33, :18:14]
  reg  wb_valid;	// @[playground/src/WB_stage.scala:16:33]
  wire _WB_IO_ready_output = ~wb_valid | wb_ready_go;	// @[playground/src/WB_stage.scala:16:33, :17:33, :19:{18,28}]
  wire _WB_to_rf_wen_output = WB_IO_bits_wen & wb_valid;	// @[playground/src/WB_stage.scala:16:33, :28:33]
  always @(posedge clock) begin	// @[<stdin>:1447:11]
    if (reset)	// @[<stdin>:1447:11]
      wb_valid <= 1'h0;	// @[playground/src/WB_stage.scala:16:33]
    else if (_WB_IO_ready_output)	// @[playground/src/WB_stage.scala:19:28]
      wb_valid <= WB_IO_valid;	// @[playground/src/WB_stage.scala:16:33]
  end // always @(posedge)
  DPI_stage DPI_stage (	// @[playground/src/WB_stage.scala:35:23]
    .clock            (clock),
    .reset            (reset),
    .DPI_wb_valid     (wb_valid),	// @[playground/src/WB_stage.scala:16:33]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1446:3, playground/src/WB_stage.scala:19:28]
  assign WB_to_rf_waddr = WB_IO_bits_rd;	// @[<stdin>:1446:3]
  assign WB_to_rf_wdata = WB_IO_bits_result;	// @[<stdin>:1446:3]
  assign WB_to_rf_wen = _WB_to_rf_wen_output;	// @[<stdin>:1446:3, playground/src/WB_stage.scala:28:33]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1446:3]
  assign WB_debug_wdata = WB_IO_bits_result;	// @[<stdin>:1446:3]
  assign WB_debug_wen = _WB_to_rf_wen_output;	// @[<stdin>:1446:3, playground/src/WB_stage.scala:28:33]
endmodule

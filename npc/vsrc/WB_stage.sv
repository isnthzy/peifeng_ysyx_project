...
module WB_stage(	// @[<stdin>:1434:3]
  input         clock,	// @[<stdin>:1435:11]
                reset,	// @[<stdin>:1436:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1435:11]
    if (reset)	// @[<stdin>:1435:11]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1434:3, playground/src/WB_stage.scala:20:28]
  assign WB_to_rf_waddr = WB_IO_bits_rd;	// @[<stdin>:1434:3]
  assign WB_to_rf_wdata = _WB_to_rf_wdata_output;	// @[<stdin>:1434:3, playground/src/WB_stage.scala:36:22]
  assign WB_to_rf_wen = _WB_to_rf_wen_output;	// @[<stdin>:1434:3, playground/src/WB_stage.scala:38:33]
  assign WB_to_if_epc_wen = WB_IO_bits_pc_sel;	// @[<stdin>:1434:3]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1434:3]
  assign WB_debug_wdata = _WB_to_rf_wdata_output;	// @[<stdin>:1434:3, playground/src/WB_stage.scala:36:22]
  assign WB_debug_wen = _WB_to_rf_wen_output;	// @[<stdin>:1434:3, playground/src/WB_stage.scala:38:33]
endmodule

...
module WB_stage(	// @[<stdin>:1528:3]
  input         clock,	// @[<stdin>:1529:11]
                reset,	// @[<stdin>:1530:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:1529:11]
    if (reset)	// @[<stdin>:1529:11]
  ...
  assign WB_IO_ready = _WB_IO_ready_output;	// @[<stdin>:1528:3, playground/src/WB_stage.scala:20:28]
  assign WB_to_id_rf_waddr = WB_IO_bits_rd;	// @[<stdin>:1528:3]
  assign WB_to_id_rf_wdata = WB_IO_bits_result;	// @[<stdin>:1528:3]
  assign WB_to_id_rf_wen = _WB_to_id_rf_wen_output;	// @[<stdin>:1528:3, playground/src/WB_stage.scala:28:39]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1528:3]
  assign WB_debug_wdata = WB_IO_bits_result;	// @[<stdin>:1528:3]
  assign WB_debug_wen = _WB_to_id_rf_wen_output;	// @[<stdin>:1528:3, playground/src/WB_stage.scala:28:39]
endmodule

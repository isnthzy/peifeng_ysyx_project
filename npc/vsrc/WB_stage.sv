...
module WB_stage(	// @[<stdin>:1236:3]
  input         clock,	// @[<stdin>:1237:11]
                reset,	// @[<stdin>:1238:11]
  ...
  wire        _Csrfile_io_out_wen;	// @[playground/src/WB_stage.scala:21:21]
  wire [31:0] _Csrfile_io_out;	// @[playground/src/WB_stage.scala:21:21]
  wire [31:0] _WB_to_id_wdata_output =
    _Csrfile_io_out_wen ? _Csrfile_io_out : WB_IO_bits_result;	// @[playground/src/WB_stage.scala:21:21, :31:22]
  CsrFile Csrfile (	// @[playground/src/WB_stage.scala:21:21]
    .clock       (clock),
    .reset       (reset),
  ...
  dpi_ebreak dpi_ebreak (	// @[playground/src/WB_stage.scala:38:24]
    .clock       (clock),
    .reset       (reset),
    .ebreak_flag (WB_IO_bits_ebreak_flag),
    .pc          (WB_IO_bits_nextpc),
    .ret_reg     (_WB_to_id_wdata_output)	// @[playground/src/WB_stage.scala:31:22]
  );
  assign WB_to_id_waddr = WB_IO_bits_rd;	// @[<stdin>:1236:3]
  assign WB_to_id_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1236:3, playground/src/WB_stage.scala:31:22]
  assign WB_to_id_wen = WB_IO_bits_wen;	// @[<stdin>:1236:3]
  assign WB_to_if_epc_wen = WB_IO_bits_pc_sel;	// @[<stdin>:1236:3]
  assign WB_debug_waddr = WB_IO_bits_rd;	// @[<stdin>:1236:3]
  assign WB_debug_wdata = _WB_to_id_wdata_output;	// @[<stdin>:1236:3, playground/src/WB_stage.scala:31:22]
  assign WB_debug_wen = WB_IO_bits_wen;	// @[<stdin>:1236:3]
endmodule

...
module IF_stage(	// @[<stdin>:3:3]
  ...
  wire        _IF_r_ready_output = 1'h1;	// @[<stdin>:3:3, playground/src/IF_stage.scala:33:13]
  ...
  reg  [31:0] if_pc;	// @[playground/src/IF_stage.scala:43:26]
  wire [31:0] if_snpc = if_pc + 32'h4;	// @[playground/src/IF_stage.scala:43:26, :44:33, :50:20]
  wire [31:0] if_dnpc =
    IF_for_ex_epc_taken
      ? IF_for_ex_epc_target
      : IF_for_id_Br_J_taken ? IF_for_id_Br_J_target : IF_for_ex_Br_B_target;	// @[playground/src/IF_stage.scala:40:17, :45:33, :51:17]
  wire [31:0] if_nextpc =
    IF_for_id_Br_J_taken | IF_for_ex_Br_B_taken | IF_for_ex_epc_taken ? if_dnpc : if_snpc;	// @[playground/src/IF_stage.scala:44:33, :45:33, :46:33, :52:{18,28}]
  reg         arvalidReg;	// @[playground/src/IF_stage.scala:57:25]
  reg  [31:0] araddrReg;	// @[playground/src/IF_stage.scala:58:24]
  reg         ReadRequstState;	// @[playground/src/IF_stage.scala:61:30]
  ...
  wire [31:0] if_inst = inst_is_valid ? IF_r_bits_data : 32'h0;	// @[playground/src/IF_stage.scala:24:35, :48:36, :83:18, :84:12]
  wire        _GEN = ReadRequstState & IF_ar_ready;	// @[playground/src/IF_stage.scala:61:30, :62:33, :67:39, :68:22, :69:22]
  always @(posedge clock) begin	// @[<stdin>:4:11]
    if (reset) begin	// @[<stdin>:4:11]
      if_valid <= 1'h0;	// @[playground/src/IF_stage.scala:29:33]
      if_pc <= 32'h7FFFFFFC;	// @[playground/src/IF_stage.scala:43:26]
      arvalidReg <= 1'h0;	// @[playground/src/IF_stage.scala:29:33, :57:25]
      araddrReg <= 32'h0;	// @[playground/src/IF_stage.scala:48:36, :58:24]
      ReadRequstState <= 1'h0;	// @[playground/src/IF_stage.scala:29:33, :61:30]
    end
    else begin	// @[<stdin>:4:11]
      if_valid <= if_ready_go | if_valid;	// @[playground/src/IF_stage.scala:29:33, :30:33, :32:20, :33:13]
      if (if_ready_go)	// @[playground/src/IF_stage.scala:30:33]
        if_pc <= if_nextpc;	// @[playground/src/IF_stage.scala:43:26, :46:33]
      arvalidReg <= ~_GEN & (~ReadRequstState | arvalidReg);	// @[playground/src/IF_stage.scala:57:25, :61:30, :62:{23,33}, :65:15, :67:39, :68:22, :69:22, :70:17]
      if (ReadRequstState) begin	// @[playground/src/IF_stage.scala:61:30]
      end
      else	// @[playground/src/IF_stage.scala:61:30]
        araddrReg <= if_nextpc;	// @[playground/src/IF_stage.scala:46:33, :58:24]
      ReadRequstState <= ~_GEN;	// @[playground/src/IF_stage.scala:61:30, :62:33, :67:39, :68:22, :69:22]
    end
  end // always @(posedge)
  assign IF_to_id_valid = ~if_flush & if_valid & if_ready_go;	// @[<stdin>:3:3, playground/src/IF_stage.scala:26:30, :29:33, :30:33, :35:22]
  assign IF_to_id_bits_nextpc = if_nextpc;	// @[<stdin>:3:3, playground/src/IF_stage.scala:46:33]
  assign IF_to_id_bits_pc = if_pc;	// @[<stdin>:3:3, playground/src/IF_stage.scala:43:26]
  assign IF_to_id_bits_inst = if_inst;	// @[<stdin>:3:3, playground/src/IF_stage.scala:48:36]
  assign IF_ar_valid = arvalidReg;	// @[<stdin>:3:3, playground/src/IF_stage.scala:57:25]
  assign IF_ar_bits_addr = araddrReg;	// @[<stdin>:3:3, playground/src/IF_stage.scala:58:24]
  ...
  assign IF_aw_bits_addr = 32'h0;	// @[<stdin>:3:3, playground/src/IF_stage.scala:48:36]
  ...
  assign IF_w_bits_data = 32'h0;	// @[<stdin>:3:3, playground/src/IF_stage.scala:48:36]

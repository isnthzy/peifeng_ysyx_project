...
module IF_stage(	// @[<stdin>:11:3]
  ...
  wire        if_ready_go = 1'h1;	// @[playground/src/IF_stage.scala:14:33, :15:14]
  ...
  always @(posedge clock) begin	// @[<stdin>:12:11]
    if (reset) begin	// @[<stdin>:12:11]
      ...
      if_valid <= ~reset | if_valid;	// @[playground/src/IF_stage.scala:13:33, :16:{20,30}, :17:13]
  ...
  read_inst Fetch (	// @[playground/src/IF_stage.scala:26:23]
    ...
    .pc     (REGpc),	// @[playground/src/IF_stage.scala:22:24]

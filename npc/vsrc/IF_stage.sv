...
module IF_stage(	// @[<stdin>:80:3]
  ...
  wire        if_inst_is_valid = IF_r_valid;	// @[playground/src/IF_stage.scala:18:38]
  ...
  wire        if_clog = ~if_inst_is_valid & if_valid;	// @[playground/src/IF_stage.scala:17:29, :18:38, :23:33, :25:{13,30}]
  wire        if_ready_go = ~if_clog;	// @[playground/src/IF_stage.scala:17:29, :23:33, :24:33, :26:19]
  wire        _IF_IO_ready_output = ~if_valid | if_ready_go & IF_to_id_ready;	// @[playground/src/IF_stage.scala:23:33, :24:33, :27:{18,28,43}]
  ...
  always @(posedge clock) begin	// @[<stdin>:81:11]
    if (reset)	// @[<stdin>:81:11]
      if_valid <= 1'h0;	// @[playground/src/IF_stage.scala:23:33]
    else if (_IF_IO_ready_output)	// @[playground/src/IF_stage.scala:27:28]
      if_valid <= IF_IO_valid;	// @[playground/src/IF_stage.scala:23:33]
  end // always @(posedge)
  assign IF_IO_ready = _IF_IO_ready_output;	// @[<stdin>:80:3, playground/src/IF_stage.scala:27:28]
  assign IF_to_id_valid = ~if_flush & if_valid & if_ready_go;	// @[<stdin>:80:3, playground/src/IF_stage.scala:20:30, :23:33, :24:33, :31:22]

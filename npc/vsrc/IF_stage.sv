...
module IF_stage(	// @[<stdin>:11:3]
  ...
  always @(posedge clock) begin	// @[<stdin>:12:11]
    if (reset) begin	// @[<stdin>:12:11]
      ...
      AxiValidOpen <= 1'h0;	// @[playground/src/IF_stage.scala:18:34, :19:37]
      if_pc <= 32'h7FFFFFFC;	// @[playground/src/IF_stage.scala:32:26]
    end
    else begin	// @[<stdin>:12:11]
      ResetNReg <= 1'h1;	// @[playground/src/IF_stage.scala:18:34, :20:21]
      AxiValidOpen <= ResetNReg;	// @[playground/src/IF_stage.scala:18:34, :19:37]
      if (if_ready_go)	// @[playground/src/IF_stage.scala:22:33]

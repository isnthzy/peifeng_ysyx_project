...
module IF_stage(	// @[<stdin>:87:3]
  ...
  assign IF_IO_ready = _IF_IO_ready_output;	// @[<stdin>:87:3, playground/src/IF_stage.scala:25:28]
  assign IF_to_id_valid = ~if_flush & if_valid & if_ready_go;	// @[<stdin>:87:3, playground/src/IF_stage.scala:19:30, :22:33, :23:33, :29:22]
  assign IF_to_id_bits_nextpc = IF_IO_bits_nextpc;	// @[<stdin>:87:3]
  assign IF_to_id_bits_pc = IF_IO_bits_pc;	// @[<stdin>:87:3]
  assign IF_to_id_bits_inst = if_inst;	// @[<stdin>:87:3, playground/src/IF_stage.scala:16:36]
endmodule

...
module IF_stage(	// @[<stdin>:11:3]
  ...
  always @(posedge clock) begin	// @[<stdin>:12:11]
    if (reset) begin	// @[<stdin>:12:11]
      ...
      if (if_ready_go)	// @[playground/src/IF_stage.scala:29:33]
        if_pc <= if_nextpc;	// @[playground/src/IF_stage.scala:42:26, :45:33]
    end
  end // always @(posedge)
  read_inst Fetch (	// @[playground/src/IF_stage.scala:61:19]
    .clock     (clock),
    .reset     (reset),
  ...
  assign IF_to_id_valid = ~if_flush & if_valid & if_ready_go;	// @[<stdin>:11:3, playground/src/IF_stage.scala:25:30, :28:33, :29:33, :34:22]

...
module PreIF_s(	// @[<stdin>:3:3]
  ...
  wire        _PreIF_to_if_valid_output = ~PreIF_flush & ~reset & PreIF_ready_go;	// @[playground/src/PreIF.scala:18:36, :20:33, :40:{26,48}]
  ...
  wire        is_fire = PreIF_to_if_ready & _PreIF_to_if_valid_output;	// @[playground/src/PreIF.scala:40:26, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire        _GEN = PreIF_ready_go | br_modify;	// @[playground/src/PreIF.scala:18:36, :21:31, :104:23]
  `ifndef SYNTHESIS	// @[playground/src/PreIF.scala:105:11]
    always @(posedge clock) begin	// @[playground/src/PreIF.scala:105:11]
      if ((`PRINTF_COND_) & _GEN & ~reset)	// @[playground/src/PreIF.scala:104:23, :105:11]
        $fwrite(32'h80000002, "PreIF: pc=%x, nextpc=%x , is_fire=%d\n", PreIF_pc,
                PreIF_nextpc, is_fire);	// @[playground/src/PreIF.scala:47:29, :50:36, :105:11, src/main/scala/chisel3/util/Decoupled.scala:52:35]
    end // always @(posedge)
  `endif // not def SYNTHESIS
  wire        _GEN_0 = ReadRequstState & PreIF_ar_ready;	// @[playground/src/PreIF.scala:65:30, :72:45, :73:25, :74:22]
  always @(posedge clock) begin	// @[<stdin>:4:11]
    if (reset) begin	// @[<stdin>:4:11]
      PreIF_pc <= 32'h80000000;	// @[playground/src/PreIF.scala:47:29]
      arvalidReg <= 1'h0;	// @[playground/src/PreIF.scala:35:35, :61:25]
      araddrReg <= 32'h0;	// @[playground/src/PreIF.scala:62:24]
      ReadRequstState <= 1'h0;	// @[playground/src/PreIF.scala:35:35, :65:30]
    end
    else begin	// @[<stdin>:4:11]
      if (_GEN)	// @[playground/src/PreIF.scala:104:23]
        PreIF_pc <= PreIF_nextpc;	// @[playground/src/PreIF.scala:47:29, :50:36]
      if (ReadRequstState) begin	// @[playground/src/PreIF.scala:65:30]
        arvalidReg <= ~_GEN_0 & arvalidReg;	// @[playground/src/PreIF.scala:61:25, :65:30, :72:45, :73:25, :74:22, :75:17]
        ReadRequstState <= ~_GEN_0 & ReadRequstState;	// @[playground/src/PreIF.scala:65:30, :72:45, :73:25, :74:22]
      end
  ...
  assign PreIF_to_if_valid = _PreIF_to_if_valid_output;	// @[<stdin>:3:3, playground/src/PreIF.scala:40:26]

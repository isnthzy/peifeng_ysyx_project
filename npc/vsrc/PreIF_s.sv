...
module PreIF_s(	// @[<stdin>:3:3]
  ...
  wire        _PreIF_to_if_valid_output = ~PreIF_flush & resetn;	// @[playground/src/PreIF.scala:18:28, :26:33, :29:26]
  ...
  wire        _GEN = PreIF_to_if_ready & _PreIF_to_if_valid_output;	// @[playground/src/PreIF.scala:29:26, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  wire        _GEN_0 = _GEN & resetn;	// @[playground/src/PreIF.scala:18:28, :93:24, src/main/scala/chisel3/util/Decoupled.scala:52:35]
  `ifndef SYNTHESIS	// @[playground/src/PreIF.scala:94:11]
    always @(posedge clock) begin	// @[playground/src/PreIF.scala:94:11]
      if ((`PRINTF_COND_) & _GEN_0 & ~reset)	// @[playground/src/PreIF.scala:93:24, :94:11]
        $fwrite(32'h80000002, "PreIF: pc=%x, nextpc=%x , is_fire=%d\n", PreIF_pc,
                PreIF_nextpc, _GEN);	// @[playground/src/PreIF.scala:36:29, :39:36, :94:11, src/main/scala/chisel3/util/Decoupled.scala:52:35]
    end // always @(posedge)
  `endif // not def SYNTHESIS
  wire        _GEN_1 = ReadRequstState & PreIF_ar_ready;	// @[playground/src/PreIF.scala:54:30, :61:45, :62:25, :63:22]
  always @(posedge clock) begin	// @[<stdin>:4:11]
    if (reset) begin	// @[<stdin>:4:11]
      ...
      if (_GEN_0)	// @[playground/src/PreIF.scala:93:24]
        PreIF_pc <= PreIF_nextpc;	// @[playground/src/PreIF.scala:36:29, :39:36]
      if (ReadRequstState) begin	// @[playground/src/PreIF.scala:54:30]
        arvalidReg <= ~_GEN_1 & arvalidReg;	// @[playground/src/PreIF.scala:50:25, :54:30, :61:45, :62:25, :63:22, :64:17]
        ReadRequstState <= ~_GEN_1 & ReadRequstState;	// @[playground/src/PreIF.scala:54:30, :61:45, :62:25, :63:22]
      end
  ...
  assign PreIF_to_if_valid = _PreIF_to_if_valid_output;	// @[<stdin>:3:3, playground/src/PreIF.scala:29:26]

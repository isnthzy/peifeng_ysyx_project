...
void init_elf(const char* elf_file) {
    ...
    if (result != section_header_entry_count) {
        Log("读取节头表失败!\n");
        fclose(file);
        return;
    }
    ...
    if (result != symbol_count) {
        Log("读取符号表失败!\n");
        fclose(file);
        return;
    }
    // 遍历符号表，筛选出类型为FUNC的符号
    ELF_Func elf_func[symbol_count];
    size_t func_cnt = 0;
    for (size_t i = 0; i < symbol_count; ++i) {
        if (ELF32_ST_TYPE(symbols[i].st_info)== STT_FUNC) {
            ...
            strncpy(elf_func[func_cnt].func_name, symbol_name, sizeof(elf_func[func_cnt].func_name) - 1);
            elf_func[func_cnt].func_name[sizeof(elf_func[func_cnt].func_name) - 1] = '\0';
            // 获取符号的地址
            elf_func[func_cnt].value = symbols[i].st_value;
            elf_func[func_cnt].size = symbols[i].st_size;

            printf("Function: %s\nAddress: 0x%lx %ld(Dec) %lx(Hex)\n",
                   elf_func[func_cnt].func_name,
                   elf_func[func_cnt].value,
                   elf_func[func_cnt].size,
                   elf_func[func_cnt].size);

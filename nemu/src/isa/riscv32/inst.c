...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, Reg(rd) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, Reg(rd) = src1<(word_t)imm; printf("%x %x %x\n",src1,(word_t)imm,src1<(word_t)imm));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1==src2) s->dnpc=s->pc+imm; printf("%d %x %x\n",src1==src2,s->dnpc,imm));
